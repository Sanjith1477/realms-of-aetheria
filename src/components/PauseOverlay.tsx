  ...
  return (
    <div className="absolute inset-0 z-40 bg-abyss/75 flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="panel-gold clip-notch w-full max-w-5xl p-4 sm:p-6 anim-fade-up my-auto">
        ...
        <nav className="pause-actions mt-5 grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4" aria-label="Pause menu">
            <button onClick={onResume} className="pause-action pause-action-primary btn-gold clip-notch flex flex-col items-center justify-center gap-1.5 py-4 sm:py-6 text-sm sm:text-base font-black">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M8 5.2v13.6c0 1 1.1 1.6 2 1l9.2-6.8c.8-.6.8-1.8 0-2.4L10 3.8c-.9-.6-2-.1-2 1.4Z" /></svg>
              RESUME
            </button>
            <button onClick={onOpenSettings} className="pause-action btn-dark clip-notch flex flex-col items-center justify-center gap-1.5 py-4 sm:py-6 text-sm sm:text-base font-bold">
            ...
            <button onClick={onRestart} className="pause-action btn-dark clip-notch flex flex-col items-center justify-center gap-1.5 py-4 sm:py-6 text-sm sm:text-base font-bold">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M20 11a8 8 0 1 0 1 4" /><path d="M20 4v7h-7" /></svg>
              RESTART
            </button>
            <button onClick={onMenu} className="pause-action pause-action-danger btn-dark clip-notch flex flex-col items-center justify-center gap-1.5 py-4 sm:py-6 text-sm sm:text-base font-bold">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M6 4h12v16H6z" /><path d="m10 9 4 3-4 3" /></svg>
              ABANDON
            </button>
        </nav>
